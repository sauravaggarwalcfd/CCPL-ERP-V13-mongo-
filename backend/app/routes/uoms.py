...
async def get_uom_groups():
    ...
    return [
        {
            "code": group.value,
            "name": UOM_GROUP_NAMES[group],
            "value": group.value,
            "label": UOM_GROUP_NAMES[group]
        }
        for group in UOMGroup
    ]
...
async def create_uom(data: UOMCreate):
    ...
    if data.is_base_uom:
        ...
        if existing_base:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail=f"Base UOM already exists for group '{data.uom_group.value}': {existing_base.uom_code}"
            )

    # Get group display name
    group_name = UOM_GROUP_NAMES.get(data.uom_group, data.uom_group.value)
...
async def update_uom(code: str, data: UOMUpdate):
    ...
    if "uom_group" in update_data:
        update_data["group_name"] = UOM_GROUP_NAMES.get(
            update_data["uom_group"],
            update_data["uom_group"].value
        )
