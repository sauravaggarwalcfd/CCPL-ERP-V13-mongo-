...
def size_to_response(size: SizeMaster) -> SizeResponse:
    ...
    return SizeResponse(
        ...
        size_group=size.size_group.value,
...
async def get_size_groups():
    ...
    return [
        {
            "code": group.value,
            "name": SIZE_GROUP_NAMES[group],
            "value": group.value,
            "label": SIZE_GROUP_NAMES[group]
        }
...
async def create_size(data: SizeCreate):
    ...
    group_name = SIZE_GROUP_NAMES.get(data.size_group, data.size_group.value)
...
async def update_size(code: str, data: SizeUpdate):
    ...
    if "size_group" in update_data:
        update_data["group_name"] = SIZE_GROUP_NAMES.get(
            update_data["size_group"],
            update_data["size_group"].value
        )
