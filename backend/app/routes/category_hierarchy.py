...
async def create_sub_class(data: ItemSubClassCreate):
    ...
    item = await ItemSubClass(
        ...
        item_type=data.item_type if hasattr(data, 'item_type') and data.item_type else cat.item_type,
        sku_prefix=f"{data.item_type if hasattr(data, 'item_type') and data.item_type else cat.item_type}-{data.sub_class_code.upper()}",
        last_sequence="A0000",
        has_color=data.has_color,
        has_size=data.has_size,
        has_fabric=data.has_fabric,
                ...
                for cls in classes:
                    ...
                    for subcls in sub_classes:
                        cls_node["children"].append({
                            ...
                            "sku_prefix": f"{getattr(subcls, 'item_type', 'FG')}-{subcls.sub_class_code}",
